...
namespace wackydatabase
{
    ...
    public class WMRecipeCust : BaseUnityPlugin
    {
        ...
        { DisplayName = ModName, MinimumRequiredVersion = "1.1.1" }; // it is very picky on version number
        ...
        private static void SetPieceRecipeData(PieceData data)
        {
            ...
            if (go == null)
            {
                ...
                if (go == null)
                {
                    go = CheckforSpecialObjects(data.name);// check for special cases
...
/*
	...
	private static class HotkeyBar_UpdateIcons_Patch
	{
		...
		private static void Postfix(HotkeyBar __instance)
		{
			...
			if (((Object)((Component)__instance).get_gameObject()).get_name() != "HotKeyBar")
			{
				return;
			}
			if (!FirstInit)
			{
				FirstInit = true;
				Transform transform = __instance.m_elementPrefab.get_transform();
				GameObject obj = Object.Instantiate<GameObject>(UIconPart);
				obj.get_transform().SetParent(transform);
				((Object)obj).set_name("KGtransmute");
				obj.GetComponent<Image>().set_sprite(IconReplacement);
				obj.GetComponent<RectTransform>().set_anchoredPosition(new Vector2(-36f, -26f));
				obj.get_gameObject().SetActive(false);
			}
			if (!Object.op_Implicit((Object)(object)Player.m_localPlayer) || Player.m_localPlayer.IsDead())
			{
				return;
			}
			foreach (HotkeyBar.ElementData element in __instance.m_elements)
			{
				if (!element.m_used)
				{
					((Component)element.m_go.get_transform().Find("KGtransmute")).get_gameObject().SetActive(false);
				}
			}
			for (int i = 0; i < __instance.m_items.Count; i++)
			{
				ItemDrop.ItemData itemData = __instance.m_items[i];
				GameObject gameObject = ((Component)__instance.m_elements[itemData.m_gridPos.x].m_go.get_transform().Find("KGtransmute")).get_gameObject();
				ExtendedItemData obj2 = ItemDataExtensions.Extended(itemData);
				gameObject.SetActive(((obj2 != null) ? obj2.GetComponent<TransmogrifyKGdata>() : null) != null);
			}
		}
	}
	...
	private static class InventoryGrid_Awake_Patch
	{
		private static void Postfix(InventoryGrid __instance)
		{
			...
			if (Object.op_Implicit((Object)(object)__instance.m_elementPrefab))
			{
				Transform transform = __instance.m_elementPrefab.get_transform();
				GameObject obj = Object.Instantiate<GameObject>(UIconPart);
				obj.get_transform().SetParent(transform);
...
Texture2D val = new Texture2D(1, 1);
		...
		IconReplacement = Sprite.Create(val, new Rect(0f, 0f, (float)((Texture)val).get_width(), (float)((Texture)val).get_height()), Vector2.get_zero());
*/